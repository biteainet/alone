...
let modRegistered = !1,
  ShakeEffectOn = null === localStorage.getItem("ShakeEffectOn") || "true" === localStorage.getItem("ShakeEffectOn"),
...
const isOnMobile = /Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent),
  isOnPC = !isOnMobile;
...
class PausableDelay {
  constructor(o, n) {
    this.callback = o,
      ...
      this.timeoutId = null,
      this.isActive = !0,
      isPausedD || this.resume()
  }
  pause() {
    this.isActive && null !== this.timeoutId && (clearTimeout(this.timeoutId), this.timeoutId = null, this.timeRemaining = this.delay - (Date.now() - this.startTime), this.timeRemaining < 0 && (this.timeRemaining = 0))
  }
  resume() {
    this.isActive && null === this.timeoutId && (this.startTime = Date.now(), this.timeoutId = setTimeout((() => {
      this.complete()
    }), this.timeRemaining))
  }
  complete() {
    this.isActive && (this.isActive = !1, this.callback(), activeDelays = activeDelays.filter((o => o !== this)))
  }
  cancel() {
    this.isActive && (this.isActive = !1, null !== this.timeoutId && (clearTimeout(this.timeoutId), this.timeoutId = null), activeDelays = activeDelays.filter((o => o !== this)))
  }
}

function OnDelayIn(o, n) {
  const e = new PausableDelay(n, o);
  return activeDelays.push(e), e
}

function pauseAllDelays() {
  isPausedD || (isPausedD = !0, pauseStartTime = Date.now(), activeDelays.forEach((o => {
    o.pause()
  })), "number" == typeof currentFadeTimeout && null !== currentFadeTimeout && clearTimeout(currentFadeTimeout))
}

function resumeAllDelays() {
  isPausedD && (isPausedD = !1, pauseStartTime = null, activeDelays.forEach((o => {
    o.resume()
  })))
}

function cancelAllDelays() {
  activeDelays.forEach((o => {
    o.cancel()
  })), activeDelays = [], "number" == typeof currentFadeTimeout && null !== currentFadeTimeout && (clearTimeout(currentFadeTimeout), currentFadeTimeout = null)
}
...
function FadeOut(o, n, e = !1, t = null) {
  if (!BGorFadeOutsOn) return;
  ...
  s.id = "global-fade",
    ...
    s.style.transition = n > 0 ? `opacity ${n}ms ease-in` : "none",
    s.style.opacity = e ? "0" : "1",
    document.body.appendChild(s),
    s.offsetWidth,
    n > 0 ? (s.style.opacity = e ? "1" : "0", "function" == typeof t && setTimeout((() => t()), n), e || setTimeout((() => s.remove()), n)) : (s.style.opacity = e ? "1" : "0", "function" == typeof t && t(), e || s.remove())
}
...
function Shake(o, n) {
  if (ShakeEffectOn) {
    ...
    const s = l => {
      ...
      const c = l - i,
        a = (n - c) / n,
        r = (2 * Math.random() - 1) * o * a,
        u = (2 * Math.random() - 1) * o * a;
      e.style.transform = `translate(${r}px, ${u}px)`,
        c < n ? requestAnimationFrame(s) : e.style.cssText = t
    };
...
function bpmLoop(o) {
  if (!bpmCallback || isPaused) return void (animFrameId = requestAnimationFrame(bpmLoop));
  const n = 60 / bpmValue * 1e3,
    e = o - timeOffset;
  e - lastBeatTime >= n && (lastBeatTime += n, e - lastBeatTime >= n && (lastBeatTime = e), bpmCallback()), animFrameId = requestAnimationFrame(bpmLoop)
}

function onBPM(o, n) {
  offBPM(), bpmValue = o, bpmCallback = n, lastBeatTime = performance.now(), isPaused = !1, timeOffset = 0, animFrameId = requestAnimationFrame(bpmLoop)
}

function offBPM() {
  animFrameId && (cancelAnimationFrame(animFrameId), animFrameId = null), bpmCallback = null, bpmValue = 0, lastBeatTime = 0, isPaused = !1, timeOffset = 0
}

function pauseBPM() {
  isPaused || (isPaused = !0, pauseStartTime = performance.now())
}

function resumeBPM() {
  if (isPaused) {
    const o = performance.now() - pauseStartTime;
    timeOffset += o, isPaused = !1
  }
}
...
function checkForGriser(o) {
  if (o && o.classList.contains("griser")) {
    ...
    showNotification(e, "polo"), totalPolosUsed++, localStorage.setItem("totalPolosUsed", totalPolosUsed)
  }
}
...
const pictoObserver = new MutationObserver(debounce((o => {
  o.forEach((o => {
...
startObservingPicto();
const audioCache = { expire: {}, aspire: {}, sound: {} };

function initAudioContext() {
  if (!audioContext) try {
    window.AudioContext = window.AudioContext || window.webkitAudioContext, audioContext = new AudioContext, console.log("AudioContext initialized")
  } catch (o) {
    console.error("Failed to create AudioContext:", o)
  }
  return audioContext
}
...
function setupAudioContextInitialization() {
  const o = () => {
    initAudioContext(), ["click", "touchstart", "keydown"].forEach((n => document.removeEventListener(n, o)))
  };
  ["click", "touchstart", "keydown"].forEach((n => document.addEventListener(n, o, { once: !0 })))
}

function preloadBonusSounds() {
  console.log("Preloading bonus sounds..."), app.bonusarray && Array.isArray(app.bonusarray) ? (window.preloadedSounds = [], app.bonusarray.forEach(((o, n) => {
    o && (o.expire && preloadSound("expire", o.expire, n), o.aspire && preloadSound("aspire", o.aspire, n), o.sound && preloadSound("sound", o.sound, n))
  }))) : console.error("Bonus array not available for preloading")
}

function preloadSound(o, n, e) {
  if (audioCache[o][n] && audioCache[o][n].buffer) return void console.log(`Sound already cached: ${o} - ${n}`);
  const t = `${app.folder}/sound/ogg/${n}.ogg`,
    i = `${app.folder}/sound/mp3/${n}.mp3`,
    s = `${app.folder}/sound/wav/${n}.wav`;
  audioCache[o][n] = { buffer: null, paths: { ogg: t, mp3: i, wav: s }, loaded: !1, bonusIndex: e, loadFailed: !1 }, window.preloadedSounds.push({ type: o, name: n, index: e, paths: audioCache[o][n].paths }), loadSoundFile(t).then((e => audioContext ? audioContext.decodeAudioData(e) : (audioCache[o][n].rawData = e, Promise.resolve(null)))).then((e => {
    e && (audioCache[o][n].buffer = e), audioCache[o][n].loaded = !0, audioCache[o][n].format = "ogg", console.log(`Successfully preloaded ${o} sound: ${n} (OGG)`)
  })).catch((e => (console.warn(`Failed to load OGG format for ${n}, trying MP3...`, e), loadSoundFile(i).then((e => audioContext ? audioContext.decodeAudioData(e) : (audioCache[o][n].rawData = e, Promise.resolve(null)))).then((e => {
    e && (audioCache[o][n].buffer = e), audioCache[o][n].loaded = !0, audioCache[o][n].format = "mp3", console.log(`Successfully preloaded ${o} sound: ${n} (MP3)`)
  })).catch((e => (console.warn(`Failed to load MP3 format for ${n}, trying WAV...`, e), loadSoundFile(s).then((e => audioContext ? audioContext.decodeAudioData(e) : (audioCache[o][n].rawData = e, Promise.resolve(null)))).then((e => {
    e && (audioCache[o][n].buffer = e), audioCache[o][n].loaded = !0, audioCache[o][n].format = "wav", console.log(`Successfully preloaded ${o} sound: ${n} (WAV)`)
  })).catch((e => {
    isMobile || (audioCache[o][n].loadFailed = !0)
  }))))))))
}

function loadSoundFile(o) {
  return new Promise(((n, e) => {
    const t = new XMLHttpRequest;
    t.open("GET", o, !0), t.responseType = "arraybuffer", t.onload = function() {
      200 === t.status ? n(t.response) : e(new Error(`Status: ${t.status} for ${o}`))
    }, t.onerror = function() {
      e(new Error(`Network error loading ${o}`))
    }, t.send()
  }))
}

function playBonusSound(o, n, e = null) {
  if (console.log(`Attempting to play ${o} sound: ${n}`), !audioContext) try {
    initAudioContext()
  } catch (o) {
    console.error("Failed to initialize AudioContext:", o)
  }
  "suspended" === audioContext?.state && audioContext.resume().catch((o => console.warn("Couldn't resume AudioContext:", o)));
  let t,
    i = audioCache[o][n];
  if (!i && null !== e && app.bonusarray[e]) {
    const t = app.bonusarray[e];
    t[o] && (n = t[o], i = audioCache[o][n])
  }
  if (!i) return console.log(`Sound not preloaded, loading on demand: ${o} - ${n}`), preloadSound(o, n, e), i = audioCache[o][n], new Promise(((t, s) => {
    const l = setInterval((() => {
      i.loaded ? (clearInterval(l), playBonusSound(o, n, e).then(t).catch(s)) : i.loadFailed && (clearInterval(l), s(new Error(`Failed to load sound: ${n}`)))
    }), 100);
    setTimeout((() => {
      clearInterval(l), s(new Error(`Timeout waiting for sound to load: ${n}`))
    }), 5e3)
  }));
  if (isOnPC && (!i || i.loadFailed)) return boxDialog.open(`Expire "${n}.ogg" failed to play - the sound file is not loaded which means that the location of this file is incorrect or file just dont exist`, "😓"), Promise.reject(new Error(`Sound not available: ${n}`));
  if (audioContext && i.buffer) try {
    const e = audioContext.createBufferSource();
    return e.buffer = i.buffer, e.connect(audioContext.destination), e.start(0), console.log(`Playing ${o} sound ${n} using AudioContext`), Promise.resolve(e)
  } catch (o) {
    console.warn(`AudioContext playback failed: ${o.message}, falling back to Audio element`)
  } else if (i.rawData && audioContext) return audioContext.decodeAudioData(i.rawData.slice(0)).then((e => {
    i.buffer = e;
    const t = audioContext.createBufferSource();
    return t.buffer = e, t.connect(audioContext.destination), t.start(0), console.log(`Playing ${o} sound ${n} using AudioContext (just-decoded)`), t
  })).catch((e => {
    throw isOnPC && boxDialog.open(`Problem playing ${o} sound ${n}. Error: ${e.message}`, "&#128532;"), e
  }));
  t = i.format ? i.paths[i.format] : i.paths.ogg;
  const s = new Audio(t);
  return s.play().then((() => (console.log(`Successfully playing ${o} sound ${n} using Audio element`), s))).catch((e => {
    if (console.warn(`Playback failed for ${n} using Audio element:`, e), "NotAllowedError" === e.name) {
      console.log("Autoplay blocked. Will retry on next user interaction.");
      const o = () => {
        s.play().catch((o => console.error(`Still failed to play after interaction: ${n}`, o))), ["click", "touchstart", "keydown"].forEach((n => document.removeEventListener(n, o)))
      };
      return ["click", "touchstart", "keydown"].forEach((n => document.addEventListener(n, o, { once: !0 }))), Promise.reject(new Error("Waiting for user interaction"))
    }
    return isMobile || boxDialog.open(`Problem playing ${o} sound ${n}. Error: ${e.message}`, "&#128532;"), Promise.reject(e)
  }))
}
...
function handleBonusClassChanges(o) {
  ...
  if (n && e) {
    const n = o.id.split("-")[2],
      e = parseInt(n) - 1,
      t = getVersionFromURL();
    showNotification(`DEBUG: v${t} Bonus ${n} is playing!`, "bonus");
    const i = new MutationObserver((() => {
      if (!o.classList.contains("playing")) {
        showNotification(`DEBUG: v${t} Bonus ${n} has finished playing!`, "bonus-finished");
        const o = app.bonusarray[e],
          s = o?.expire;
        s ? (console.log(`Preparing to play expire sound for bonus ${n}:`, { bonusIndex: e, soundName: s, soundData: audioCache.expire[s] }), playBonusSound("expire", s, e).then((() => console.log(`Successfully played expire sound for bonus ${n}`))).catch((o => console.error(`Failed to play expire sound: ${o.message}`)))) : console.warn(`No expire sound defined for bonus ${n}`), totalBonusesWatched++, localStorage.setItem("totalBonusesWatched", totalBonusesWatched), i.disconnect()
      }
    }));
    i.observe(o, { attributes: !0, attributeFilter: ["class"] })
  }
}

function initBonusSoundSystem() {
  setupAudioContextInitialization(), preloadBonusSounds(), window.debugAudioCache = audioCache, window.playBonusSound = playBonusSound, console.log("Bonus sound system initialized")
}
"complete" !== document.readyState && "interactive" !== document.readyState || (console.log("Document already loaded - initializing bonus sound system"), initBonusSoundSystem());
const bonusObserver = new MutationObserver((o => {
  o.forEach((o => {
    if ("attributes" === o.type && "class" === o.attributeName) { handleBonusClassChanges(o.target) }
  }))
})),
...
const pictoClassObserver = new MutationObserver((o => {
  o.forEach((o => {
    if ("attributes" === o.type && "class" === o.attributeName) { checkForPictoClassChange(o.target) }
  }))
}));
...
function ChangeColorBG(o, n, e) {
  if (BGorFadeOutsOn) {
    ...
    t.classList.add("fade-bg"), t.style.position = "fixed", t.style.top = "0", t.style.left = "0", t.style.width = "100%", t.style.height = "100%", t.style.backgroundColor = o, t.style.zIndex = "-9999", t.style.transition = `opacity ${n}ms ease-out`, t.style.opacity = "1", document.body.appendChild(t), e || (setTimeout((() => {
      t.style.opacity = "0"
    }), 10), setTimeout((() => {
      t.remove()
    }), n))
...
showNotification(`DEBUG: v${getVersionFromURL()} just started!`), startObservingPictoClassChanges(), totalVersionsOppened++, localStorage.setItem("totalVersionsOppened", totalVersionsOppened), fetch("versions.xml").then((o => o.text())).then((o => {
  let n = (new DOMParser).parseFromString(o, "text/xml").getElementsByTagName("line"),
    e = document.getElementById("sp-select"),
    t = "";
  for (let o = 0; o < n.length; o++) {
    let i = document.createElement("div");
    i.classList.add("sp-line"), e.appendChild(i);
    let s = n[o].getElementsByTagName("icon");
    for (let o = 0; o < s.length; o++) {
      let n = s[o].getAttribute("ID"),
        e = s[o].getAttribute("color"),
        l = s[o].getAttribute("name"),
        c = s[o].getAttribute("customIcon"),
        a = s[o].getAttribute("locked"),
        r = s[o].getAttribute("ignored"),
        u = "true" === s[o].getAttribute("customName"),
        d = "true" === s[o].getAttribute("hidden"),
        f = document.createElement("div");
      f.classList.add("vicon"), f.id = "icon" + n, f.dataset.name = l, f.dataset.color = e, f.dataset.locked = a;
      let P = document.createElement("div");
      if (c ? (P = document.createElement("img"), P.id = "customicon", P.src = c, P.alt = l) : P.classList.add("img"), f.appendChild(P), f.innerHTML += u ? `<div id="customIconName" style="color: ${e};">${l}</div>` : '<div class="txt"></div>', r && f.classList.add("ignored"), "true" !== a) f.innerHTML += '\n            <div class="bul">\n              <svg class="icn-svg"><use xlink:href="#ic-check"></use></svg>\n            </div>\n          ', i.appendChild(f);
      else {
        f.classList.add("locked"), f.innerHTML += '\n            <div class="bul">\n              <svg class="icn-svg"><use xlink:href="#ic-lock"></use></svg>\n            </div>\n          ';
        let o = document.createElement("div");
        o.classList.add("locked-vicon"), o.appendChild(f), i.appendChild(o), o.addEventListener("click", (o => {
          o.preventDefault(), showNotification("Trying to open locked icon " + n), showNotification("Trying to open locked icon", "locked"), document.querySelectorAll('.vicon[data-locked="true"]').forEach((o => {
            o.classList.remove("open", "clicked")
          }))
        }))
      }
      d && (f.style.display = "none"), t += `--colV${n}: ${e};\n`;
      let g = JSON.parse(localStorage.getItem("iconColors")) || {};
      g[`colV${n}`] = e, localStorage.setItem("iconColors", JSON.stringify(g));
      let p = document.querySelector(".locked-vicon");
      f.addEventListener("click", (function(o) {
        p ? (o.preventDefault(), showNotification("Trying to open locked icon " + n), showNotification("Trying to open locked icon", "locked"), RemoveElement("lock-all"), RemoveElement("fade-all"), document.querySelectorAll('.vicon[data-locked="true"]').forEach((o => {
          o.classList.remove("open", "clicked")
        }))) : (CreateElement("lock-all", "div"), CreateElement("fade-all", "div"), OnDelayIn(900, (() => {
          window.location.href = "app.html?v=" + n
        })))
      }))
    }
  }
...
function QuickMessage(o, n = 2e3) {
  if (LyricsOn) {
    const e = document.createElement("div");
    e.classList.add("quickmessage"), e.innerHTML = o, e.style.position = "fixed", e.style.left = "50%", e.style.bottom = "50px", e.style.transform = "translateX(-50%) translateY(20px)", e.style.zIndex = "9999", e.style.opacity = "0", e.style.transition = "opacity 0.3s ease, transform 0.3s ease, bottom 0.3s ease", document.body.appendChild(e), messageQueue.unshift(e), updateMessagePositions(), setTimeout((() => {
      e.style.opacity = "1", e.style.transform = "translateX(-50%) translateY(0px)"
    }), 10), setTimeout((() => {
      e.style.opacity = "0", e.style.transform = "translateX(-50%) translateY(20px)", setTimeout((() => {
...
function OnlyOnce(o, n) {
  let e = JSON.parse(localStorage.getItem("executedItems")) || {};
  e[o] || (n(), e[o] = !0, localStorage.setItem("executedItems", JSON.stringify(e)))
}

function ClearItemID(o) {
  let n = JSON.parse(localStorage.getItem("executedItems")) || {},
    e = JSON.parse(localStorage.getItem("storage")) || {};
  n[o] && (delete n[o], localStorage.setItem("executedItems", JSON.stringify(n))), e[o] && (delete e[o], localStorage.setItem("storage", JSON.stringify(e)))
}

function CleanColorBG() {
  document.querySelectorAll(".fade-bg").forEach((o => { o.remove() }))
}

function renameFile(o) { return o }

function goto(o) { window.location.href = `app.html?v=${o}` }

function getPoloSpriteImage() { return "polo-sprite.png" }

function ReplacePoloSprite(o) { getPoloSpriteImage = () => o }

function ReplacePictoID(o, n) {
  const e = document.getElementById(`picto${o}`);
  e && (e.setAttribute("data-picto-num", n), e.id = `picto${n}`)
}
...
function ChangeMenuTitle(o) {
  ...
  n && (n.textContent = o), e && (e.textContent = o)
}
...
function Call(o) {
  ...
  n[o] || (n[o] = { callbacks: [], active: !1 }), n[o].active = !0, n[o].callbacks.forEach((o => { "function" == typeof o && o() })), localStorage.setItem("storage", JSON.stringify(n))
}
...
function OpenModCredits() {
  boxPopup.open({
    ...
    onBoxOpenEnd: function() {
      boxPopup.$popup.find(".icon.bt.bt-round.bt-44").on("click", (function() { boxPopup.close() }))
    },
...
function loadCredits() {
  fetch("mod-credits/credits.xml").then((o => o.text())).then((o => (new window.DOMParser).parseFromString(o, "text/xml"))).then((o => {
    const n = o.querySelectorAll("page"),
      e = document.querySelector(".gallery-container .gallery-slider");
    e.innerHTML = "", n.forEach((o => {
      let n = o.getAttribute("text"),
        t = o.getAttribute("title");
      const i = o.getAttribute("image"),
        s = "true" === o.getAttribute("noImage");
      n = n.replace(/\(\!line\)/g, "[[BR]]"), n = n.replace(/\(\!hr\)/g, "<hr>"), n = n.replace(/\(\!bold\/(.*?)\)/g, "<b>$1</b>"), n = n.replace(/\(\!underline\/(.*?)\)/g, "<u>$1</u>"), n = n.replace(/\(\!italic\/(.*?)\)/g, "<i>$1</i>"), n = n.replace(/\(\!color\/#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})\/(.*?)\)/g, '<span style="color:#$1;">$2</span>'), n = n.replace(/\(\!size\/(\d+px)\/(.*?)\)/g, '<span style="font-size:$1;">$2</span>'), n = n.replace(/\(\!emoji\/(.*?)\)/g, ((o, n) => `<img src="mod-credits/emoji/${n}" class="credit-emoji" />`)), n = n.replace(/\(\!button\/#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})\/(.*?)\/(https?:\/\/[^\s]+)\/(.*?)\)/g, ((o, n, e, t, i) => `<button class="credit-button" style="background-color:#${n};" onclick="confirmNavigation('${t}')">\n                            <div class="center">\n                                    <img src="mod-credits/icons/${e}" class="button-icon"> \n                                    <span class="button-text">${i}</span>\n                                </button>\n                            </div>`)), n = n.replace(/\(\!youtube\/(.*?)\)/g, ((o, n) => {
        let e = "";
      ...
      })), n = n.replace(/\(!hyperlink\/((?:https?:\/\/)[^\/\s]+(?:\/[^\s#]*)*)\/(?:#([0-9a-fA-F]{3,6})\/)?(.+?)\)/g, ((o, n, e, t) => {
        const i = Boolean(e);
        return `<a href="#" class="credit-link"${i?` style="color: #${e}"`:""}${i?' data-hover-opacity="true"':""} onclick="confirmNavigation('${n}')">${t}</a>`
      })), n = n.replace(/\(\!img\/(.*?)\)/g, '<img src="mod-credits/$1" class="credit-inline-image">'), n = n.replace(/(^|\[\[BR\]\])[ \t]*- /g, "$1• "), n = n.replace(/\[\[BR\]\]/g, "<br>"), n = n.replace(/\(\!gradient\/#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})\/#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})\/(.*?)\)/g, '<span class="gradient-text" style="--start:#$1; --end:#$2;">$3</span>');
      const l = document.createElement("div");
      if (l.classList.add("gallery-slide"), !s) {
        const o = document.createElement("img");
        o.id = "credits-images", o.src = i, l.appendChild(o)
      }
      const c = document.createElement("div");
      c.className = "title", c.innerHTML = t, l.appendChild(c);
      const a = document.createElement("div");
      a.id = "text-credits", a.innerHTML = n, l.appendChild(a), e.appendChild(l)
    }))
  })).catch((o => {}))
}

function confirmNavigation(o) {
  boxDialog.open(`You are about to visit: <b>${o}</b><br>Do you want to continue?`, "You are leaving!", [STR("bt.sure"), STR("bt.cancel")], [() => { openURL(o) }, () => { boxDialog.close() }])
}
...
window.onload = function() { loadCredits() };
...
function showSlide(o) {
  ...
  currentSlideCredits = o >= n.length ? 0 : o < 0 ? n.length - 1 : o;
  const t = 100 * -currentSlideCredits;
  e.style.transform = `translateX(${t}%)`
}

function nextSlide() { showSlide(currentSlideCredits + 1) }

function prevSlide() { showSlide(currentSlideCredits - 1) }
...
const secretE = "https://upload.wikimedia.org/wikipedia/commons/thumb/f/fb/718smiley.svg/1200px-718smiley.svg.png";
...
function resetEverything() {
  OnDelayIn(300, (() => {
    localStorage.clear(), boxDialog.open("All data has been erased!<br>\n             The game will now reset to its first-time launch state.", "Info", !1), OnDelayIn(5500, (() => { location.reload() }))
  }))
}

function showAchiNotificationDot() {
  let o = document.querySelector(".notification__achi");
  o && (achiNotificationDot > 0 ? (o.classList.add("show"), o.innerText = achiNotificationDot >= 1e6 ? (achiNotificationDot / 1e6).toFixed(1).replace(/\.0$/, "") + "M" : achiNotificationDot >= 1e3 ? (achiNotificationDot / 1e3).toFixed(1).replace(/\.0$/, "") + "K" : achiNotificationDot) : (o.classList.remove("show"), o.innerText = ""))
}

function resetAchievements() {
  const o = `achievements_${RegisterMod}`;
  OnDelayIn(300, (() => {
    localStorage.getItem(o) ? (localStorage.removeItem(o), boxDialog.open(`All achievements in <b>${RegisterMod}</b> have been reset.<br>\n                 The game will restart in 2 seconds.`, "Info", !1), OnDelayIn(2500, (() => { location.reload() }))) : boxDialog.open("No achievements found for this mod.", "", [STR("bt.gotit")])
  }))
}
...
function MakeConfetti(o = 2, n = 360, e = 1, t = 200, i = 100, s, l) {
  if (ConfettiEffectOn) {
    const c = getElementPosition(`#${s}`);
    confetti({ spread: n, ticks: t, gravity: e, decay: .94, startVelocity: 20, particleCount: i, origin: c, scalar: o, shapes: ["image"], shapeOptions: { image: [{ src: l, width: 32, height: 32 }] } })
  }
}
...
function setCustomCursor(o) {
  if (CustomCursorsOn) {
    ...
    e || (e = document.createElement("div"), e.id = "custom-cursor", e.style.position = "fixed", e.style.width = "32px", e.style.height = "32px", e.style.pointerEvents = "none", e.style.zIndex = "9999", e.style.transform = "translate(-50%, -50%)", document.body.appendChild(e)), e.style.background = `url('${o}') no-repeat center`, e.style.backgroundSize = "contain", document.addEventListener("mousemove", (o => {
...
function Bloom(o, n = 1e3) {
  if (BGorFadeOutsOn) {
    const e = document.createElement("div");
    e.style.position = "fixed", e.style.top = "0", e.style.left = "0", e.style.width = "100%", e.style.height = "100%", e.style.zIndex = "99999999", e.style.transition = `opacity ${n}ms ease-out`, e.style.opacity = "1", e.style.pointerEvents = "none";
    const t = .4 * o;
    e.style.backdropFilter = `blur(${o}px) brightness(${t})`, document.body.appendChild(e), setTimeout((() => { e.style.opacity = "0" }), 10), setTimeout((() => { e.remove() }), n)
  }
}

function onLoop(o, n) {
  if ("number" != typeof app.looptime || app.looptime <= 0) return;
  const e = app.looptime * o;
  o <= 0 ? boxDialog.open("Loop multiplier must be greater than zero.") : setInterval(n, e)
}
...
function Zoom(o, n, e) {
  ...
  t.style.transition = "";
  const i = o / 100;
  n ? (t.style.transition = "transform 1s cubic-bezier(0.55, 0.09, 0.68, 0.53)", t.style.transform = `scale(${i})`) : e ? (t.style.transition = "transform 1s cubic-bezier(0.22, 0.61, 0.36, 1)", t.style.transform = `scale(${i})`) : t.style.transform = `scale(${i})`
}
...
OnClick("particleseffectbutton", (() => {
  ConfettiEffectOn = !ConfettiEffectOn, updateParticlesSwitch(), localStorage.setItem("ConfettiEffectOn", ConfettiEffectOn), SomeTimes(.1, (() => {
    MakeConfetti(2.5, 360, 0, 400, 50, "custom-cursor", secretE), OnDelayIn(1e3, (() => {
      MakeConfetti(2.5, 360, 0, 400, 41, "custom-cursor", secretE)
    })), OnDelayIn(2e3, (() => {
      MakeConfetti(2.5, 360, 0, 400, 29, "custom-cursor", secretE)
    })), OnDelayIn(3300, (() => {
      MakeConfetti(2.5, 360, 0, 400, 11, "custom-cursor", secretE)
    }))
  }))
})), updateParticlesSwitch(), OnClick("Cursorbuttoneffect", (() => {
  CustomCursorsOn = !CustomCursorsOn, updateCursorsEffectSwitch(), localStorage.setItem("CustomCursorsOn", CustomCursorsOn)
})), updateCursorsEffectSwitch(), OnClick("info-bgchanges", (() => {
  ...
  boxDialog.open("Are you sure you want to reset all achievement progress in this mod?<br><b>This action cannot be undone.</b>", "", [STR("bt.sure"), STR("bt.cancel")], [() => { resetAchievements() }, () => { boxDialog.close() }])
  ...
  boxDialog.open("Are you sure you want to reset everything?<br>\n         This will erase <b>all progress</b> across <b>every mod</b> and the <b>official game</b>, including:<br>\n         • Achievements<br>\n         • Mixes<br>\n         • Bonuses<br>\n         • Other saved data<br>\n         <b>This cannot be undone.</b>", "", [STR("bt.sure"), STR("bt.cancel")], [() => {
    OnDelayIn(300, (() => {
      boxDialog.open("Are you absolutely sure? <br>\n                         <b>This action is can not be undone, and you will lose everything!</b>", "Last Warning", [STR("bt.sure"), STR("bt.cancel")], [() => { resetEverything() }, () => { boxDialog.close() }])
    }))
  }, () => { boxDialog.close() }])
...
})), showAchiNotificationDot(), document.addEventListener("mousemove", (o => {
...
})), onKeyHold("I", "^", (function() { window.location.href = "js/mod-libs/index.html" })), onKeyHold("I", "&", (function() {
  const o = app.animearray.length,
    n = 680 / o;
  let e = app.bonusarray.map((o => `\n        <div>\n            <span class="icon-container">\n              <img \n                style="height:40px;border-radius:3px;vertical-align: -13px;"\n                src="${app.folder}video/${o.icon}"\n                onerror="handleMissingIcon(this)"\n              >\n            </span>\n            <b style="text-transform: capitalize;">${o.name}</b> /\n            <span class="code-icons" style="vertical-align: -10px;">\n                ${o.code.split(",").map((o=>{const e=parseInt(o)-1;return`\n                        <span class="sprite-icon" style="\n                            display:inline-block;\n                            width:34px;\n                            height:34px;\n                            background-image:url('${app.folder}img/game-picto@2x.png');\n                            background-size:680px 69px;\n                            background-position:-${e*n}px 0;\n                            background-repeat: no-repeat;\n                            margin-right:3px;\n                            border-radius: 5px;\n                        "></span>\n                      `})).join("")}\n            </span> / Loops: ${o.loop}\n        </div>\n    `)).join("");
  QuickMessage(`\n        <span style="color: ${app.col1}">BONUS INFO</span><br><hr><br>\n        <div class="modding-build">\n            ${e}\n        </div>\n    `, 1e4)
})), onKeyHold("I", "*", (() => { openAboutPop() })), onKeyHold("D", "!", (function() { toggleDarkMode() })), onKeyHold("D", "@", (function() { forceMod.killAllPolos() })), onKeyHold("D", "#", (function() { unlockAllBonus() })), onKeyHold("D", "$", (function() { location.href = "app.html?v=1" }));
let fastNavigation = !1,
  currentKeyPressed = "";

function updateMixPlayListTab() {
  const o = document.getElementById("tab-mixlist"),
    n = document.getElementById("box-mixlist");
  o.classList.add("active"), n.classList.add("show"), activeFilter("bt-mymix")
}

function handleMissingIcon(o) {
  const n = o.parentElement;
  o.remove();
  const e = document.createElement("div");
  e.innerHTML = defaultBonusSVG.trim();
  const t = e.querySelector("svg");
  t && (t.style.height = "40px", t.style.borderRadius = "3px", t.style.verticalAlign = "-13px", n.innerHTML = "", n.appendChild(t))
}
onKeyHold("D", "%", (function() {
  fastNavigation = !fastNavigation, QuickMessage("Fast Navigation is " + (fastNavigation ? "ON" : "OFF"))
})), document.addEventListener("keydown", (function(o) {
  if (fastNavigation) { "v" === o.key && document.addEventListener("keydown", (function o(n) { "v" !== n.key && (currentKeyPressed = n.key, location.href = `app.html?v=${currentKeyPressed}`), document.removeEventListener("keydown", o) }), { once: !0 }) }
}));
...
function openDatabase(o) {
  ...
  }, n.onsuccess = function(n) { o(n.target.result) }, n.onerror = function(o) {}
}
...
function registerMod() {
  let o, n;
  fetch(RegisterModIcon).then((o => {
    if (o.status, !o.ok) throw new Error("Icon file not found!");
    return o.blob()
  })).then((n => {
    if (n.size, n.type, !(n.size > 1536e3)) return o = n, fetch(RegisterModBackground);
    OnDelayIn(500, (function() { boxDialog.open("Icon is too large! Please use an image under 1.5MB.", "Icon Upload Failed") }))
  })).then((o => {
    if (o.status, !o.ok) throw new Error("Background file not found!");
    return o.blob()
  })).then((e => {
    if (e.size, e.type, !(e.size > 256e4)) return n = e, processImages(o, n);
    OnDelayIn(500, (function() { boxDialog.open("Background is too large! Please use an image under 2.5MB.", "Background Upload Failed") }))
  })).then((({ base64Icon: o, base64Background: n }) => {
    const e = indexedDB.open("ModLibraryDB", 2);
    e.onerror = function() {}, e.onsuccess = function(e) {
  ...
  })).catch((o => { boxDialog.open("Failed to load mod assets: " + o.message, "Error") }))
}
...
function processImages(o, n) {
  return new Promise(((e, t) => {
    let i, s,
      ...
      c = new FileReader;
    l.onload = () => {
      i = l.result, s && e({ base64Icon: i, base64Background: s })
    }, c.onload = () => {
      s = c.result, i && e({ base64Icon: i, base64Background: s })
    }, l.onerror = c.onerror = () => t(new Error("Failed to process images")), l.readAsDataURL(o), c.readAsDataURL(n)
  }))
}
...
function updateLastPlayed(o) {
  ...
  n.onsuccess = function(n) {
    ...
    t.onsuccess = function() {
      let o = t.result;
      o && (o.lastPlayed = Date.now(), e.put(o))
    }, t.onerror = function() {}, e.put(modData)
  }, n.onerror = function() {}
}
...
function getLastPlayed(o, n) {
  ...
  e.onsuccess = function(e) {
    ...
    }, t.onerror = function() { n("Unknown") }
  }, e.onerror = function() { n("Unknown") }
}

function loadNewButtonModsNews() {
  fetch("https://raw.githubusercontent.com/RemmieUwU/Incredimods/refs/heads/main/moreModsDataShare.json").then((o => o.json())).then((o => {
    const n = document.querySelector(".mods-buttons-warper");
    let e = document.getElementById("bt-dynamicmod");
    e || (e = document.createElement("div"), e.className = "bt bt-light with-icn", e.id = "bt-dynamicmod", n?.appendChild(e)), e.innerHTML = `\n        <div class="bck">\n          <svg class="icn-svg">\n            <use xlink:href="${o.icon}"></use>\n          </svg>\n          <div class="txt">${o.name}</div>\n        </div>\n        <a class="hitzone" href="javascript:void(0)"></a>\n      `;
    const t = e.cloneNode(!0);
    n?.replaceChild(t, e), t.querySelector(".hitzone").addEventListener("click", (() => {
      boxPopup.open({
        name: "popup-message",
        icntype: "action",
        bodyclose: !0,
        class: "modCreditsBox shadow-box",
        content: o.innerHTML || "No content available 💔",
        onBoxOpenEnd: function() {
          boxPopup.$popup.find(".icon.bt.bt-round.bt-44").on("click", (() => {
            boxPopup.close(), document.getElementById("pop-popup").classList.remove("shadow-box")
          }))
        },
        onBoxCloseStart: function() {
          boxPopup.$popup.find(".icon.bt.bt-round.bt-44").off()
...
function displayNewMods() {
  if (navigator.onLine) {
    const o = document.getElementById("modNews"),
      n = document.querySelector("#pop-modhistory .container");
    let e = document.querySelector(".banner-mods-news");
    e ? e.style.backgroundImage = "url('https://raw.githubusercontent.com/RemmieUwU/Incredimods/refs/heads/main/main-banner-min.png')" : (e = document.createElement("div"), e.classList.add("banner-mods-news"), e.style.backgroundColor = "#222", e.style.backgroundImage = "url('https://raw.githubusercontent.com/RemmieUwU/Incredimods/refs/heads/main/main-banner-min.png')", n.parentElement.insertBefore(e, n)), fetch("https://raw.githubusercontent.com/RemmieUwU/Incredimods/refs/heads/main/modData.json").then((o => o.json())).then((n => {
      o.innerHTML = "", n.mods.forEach((n => {
        const e = document.createElement("div");
        if (e.classList.add("newsModBoxes"), e.onclick = () => confirmNavigation(n.modLink), n.customCSS && "" !== n.customCSS.trim()) {
          const o = document.createElement("style");
          o.textContent = n.customCSS, e.appendChild(o)
        }
  ...
  } else { document.getElementById("modNews").innerHTML = '<div style="color:white; padding:20px; text-align:center;">No internet connection ❌<br>Please check your Wi-Fi!</div>' }
}
dbRequest.onupgradeneeded = function(o) {
  let n = o.target.result;
  n.objectStoreNames.contains("modIcons") || n.createObjectStore("modIcons", { keyPath: "name" })
}, dbRequest.onerror = function() {}, registerMod(), getLastPlayed(RegisterMod), loadNewButtonModsNews(), OnClick("tab-modhistory", (function() {
  displayNewMods(), loadNewButtonModsNews()
}));
...
if (modNews) {
  function smoothScroll() {
    currentScroll += .1 * (targetScroll - currentScroll);
    const o = modNews.scrollWidth - modNews.offsetWidth;
    currentScroll = Math.max(0, Math.min(currentScroll, o)), modNews.scrollLeft = currentScroll, Math.abs(targetScroll - currentScroll) > .5 ? requestAnimationFrame(smoothScroll) : (currentScroll = targetScroll, isScrolling = !1)
  }
  modNews.addEventListener("wheel", (o => {
    o.preventDefault();
    const n = modNews.scrollWidth - modNews.offsetWidth;
    targetScroll = Math.max(0, Math.min(targetScroll + o.deltaY, n)), isScrolling || (isScrolling = !0, smoothScroll())
  }), { passive: !1 })
}
window.addEventListener("beforeunload", (() => { localStorage.setItem("lastPage", window.location.href) }));
let AchievementON = !1;

function setAchievement(o, n, e, t, i, s, l) {
  let c = JSON.parse(localStorage.getItem(`achievements_${o}`)) || {};
  AchievementON || (AchievementON = !0), c[n] || (c[n] = { status: "locked", icon: e, name: t, description: i, type: s, progress: 0, maxProgress: l }), localStorage.setItem(`achievements_${o}`, JSON.stringify(c))
}
...
function addProgressAchievement(o, n, e) {
  ...
  if (t[n]) {
    ...
    if (void 0 === i) return;
    t[n].progress += e, t[n].progress >= i && (t[n].progress = i, t[n].status = "unlocked"), localStorage.setItem(`achievements_${o}`, JSON.stringify(t)), updateProgressUI(n, t[n].progress, i)
  }
}
...
function addProgressAchievement(o, n, e) {
  ...
  if (t[n]) {
    ...
    let i = t[n],
      s = i.maxProgress;
    if (void 0 === s) return;
    i.progress += e, i.progress >= s && (i.progress = s, i.status = "unlocked", showAchievementNotification(i.name, i.icon, i.description)), localStorage.setItem(`achievements_${o}`, JSON.stringify(t)), document.querySelectorAll(".achievement-info progress").forEach((o => {
      o.closest(".achievement").querySelector(".title").innerText === i.name && (o.value = i.progress, o.max = s)
...
function showAchievementNotification(o, n, e) {
  ...
  let s = `img/mod_achievements/${n}`;
  i.innerHTML = `\n        <img src="${s}" class="achievement-icon">\n        <div class="achievement-text">\n            <div class="title">${o}</div>\n            <div class="text">${e}</div>\n        </div>\n    `, t.appendChild(i), setTimeout((() => { i.classList.add("visible") }), 10), setTimeout((() => {
...
function displayAchievements(o) {
  ...
  if (e.innerHTML = "", 0 !== Object.keys(n).length)
    for (let o in n) {
      ...
      let s = `img/mod_achievements/${t.icon}`,
        l = "locked" === t.status ? "grayscale" : "",
        c = "locked" === t.status && "hidden" === t.type ? "???" : t.name,
        a = "locked" === t.status && "hidden" === t.type ? "???" : t.description;
    ...
    } else e.innerHTML = "<p>No achievements yet!</p>"
}
OnDelayIn(1e3, (() => { AchievementON || document.getElementById("tab-myachi").remove() }));
...
function initPictoSize() {
  ...
  920 !== o.offsetWidth ? (o.style.width = "920px", requestAnimationFrame((() => {
    applyPictoWidth(), o.style.width = n || ""
  }))) : applyPictoWidth()
}

function applyPictoWidth() {
  document.querySelectorAll(".pictoline").forEach((o => {
    const n = 85 * Array.from(o.querySelectorAll(".picto")).filter((o => "none" !== window.getComputedStyle(o).display)).length / o.parentElement.offsetWidth * 100;
    o.style.width = `${n}%`
  }))
}
...
const defaultBonusSVG = '<svg viewBox="0 0 70 70" xmlns="http://www.w3.org/2000/svg"><path fill="#ffffff" d="M 56.7 34.35Q 52.95 27.7 47.2 23.95 41.45 20.2 35.2 20.2 28.85 20.2 23.1 24 17.3 27.9 13.6 34.35 12.85 35.15 13.6 35.75 17.3 42.55 23.1 46.25 28.75 50 35.2 50 41.3 50 47.2 46.2 52.75 42.5 56.7 35.75 56.9 35.15 56.7 34.35M 35.2 28Q 38 28 40.2 30.2 42.2 32.2 42.2 35.1 42.2 38 40.2 40.1 38 42.25 35.2 42.25 32.2 42.25 30.1 40.1 28.1 38.2 28.1 35.1 28.1 32.25 30.2 30.2 32.35 28 35.2 28M 114.25 23.9Q 114.25 20.5 111.65 17.75 108.8 14.9 104.9 14.9 100.95 14.9 98.1 17.75 95.35 20.6 95.35 23.9L 95.35 29.25 92.75 29.25Q 91.85 29.25 91.1 30 90.45 30.6 90.45 31.55L 90.45 49Q 90.45 50 91.1 50.5 91.85 51.25 92.75 51.25L 117.15 51.25Q 118.1 51.25 118.75 50.5 119.45 49.95 119.45 49L 119.45 31.55Q 119.45 30.6 118.75 30 118.1 29.25 117.15 29.25L 114.25 29.25 114.25 23.9M 109.25 20.25Q 110.9 22.05 110.9 23.9L 110.9 29.25 98.85 29.25 98.85 23.9Q 98.85 21.95 100.5 20.25 102.25 18.3 104.9 18.3 107.4 18.3 109.25 20.25 Z"/></svg>',
  lockedBonusSVG = '<svg viewBox="0 0 70 70" xmlns="http://www.w3.org/2000/svg"><path fill="#ffffff" d="M 44.3 29.25L 44.3 23.9Q 44.3 20.5 41.7 17.75 38.85 14.9 34.95 14.9 31 14.9 28.15 17.75 25.4 20.6 25.4 23.9L 25.4 29.25 22.8 29.25Q 21.9 29.25 21.15 30 20.5 30.6 20.5 31.55L 20.5 49Q 20.5 50 21.15 50.5 21.9 51.25 22.8 51.25L 47.2 51.25Q 48.15 51.25 48.8 50.5 49.5 49.95 49.5 49L 49.5 31.55Q 49.5 30.6 48.8 30 48.15 29.25 47.2 29.25L 44.3 29.25M 40.95 23.9L 40.95 29.25 28.9 29.25 28.9 23.9Q 28.9 21.95 30.55 20.25 32.3 18.3 34.95 18.3 37.45 18.3 39.3 20.25 40.95 22.05 40.95 23.9M -52.7 37Q -52.5 36.4 -52.7 35.6 -56.45 28.95 -62.2 25.2 -67.95 21.45 -74.2 21.45 -80.55 21.45 -86.3 25.25 -92.1 29.15 -95.8 35.6 -96.55 36.4 -95.8 37 -92.1 43.8 -86.3 47.5 -80.65 51.25 -74.2 51.25 -68.1 51.25 -62.2 47.45 -56.65 43.75 -52.7 37M -69.2 31.45Q -67.2 33.45 -67.2 36.35 -67.2 39.25 -69.2 41.35 -71.4 43.5 -74.2 43.5 -77.2 43.5 -79.3 41.35 -81.3 39.45 -81.3 36.35 -81.3 33.5 -79.2 31.45 -77.05 29.25 -74.2 29.25 -71.4 29.25 -69.2 31.45 Z"/></svg>',
  imageCache = { bonusIcons: {} };

function initBonusesIcons() {
  console.log("Initializing bonus icons..."), app.bonusarray.forEach(((o, n) => {
    const e = document.getElementById(`bt-bonus-${n+1}`);
    if (!e) return void console.log(`Bonus element not found: bt-bonus-${n+1}`);
    const t = e.querySelector(".icon .icn-bck");
    if (!t) return void console.log(`Icon div not found in bonus element ${n+1}`);
    const i = () => {
      const i = e.classList.contains("unlock"),
        s = o.icon && "" !== o.icon.trim();
      if (console.log(`Updating icon for bonus ${n+1}: unlocked=${i}, hasIcon=${s}`), i)
        if (s) {
          const e = `${app.folder}video/${o.icon}`,
            i = imageCache.bonusIcons[e];
          i && i.loaded && i.objectURL ? (t.innerHTML = "", t.style.backgroundImage = `url('${i.objectURL}')`, t.style.backgroundSize = "cover", t.style.backgroundPosition = "center", console.log(`Using cached icon for bonus ${n+1}`)) : i && i.loadError ? (console.warn(`Using default SVG for bonus ${n+1} (icon failed to load earlier)`), t.style.backgroundImage = "", t.innerHTML = defaultBonusSVG) : (console.log(`Icon not in cache for bonus ${n+1}, loading: ${e}`), fetch(e).then((o => {
            if (!o.ok) throw new Error("Image fetch failed");
            return o.blob()
          })).then((o => {
            const i = URL.createObjectURL(o);
            imageCache.bonusIcons[e] = { loaded: !0, objectURL: i }, t.innerHTML = "", t.style.backgroundImage = `url('${i}')`, t.style.backgroundSize = "cover", t.style.backgroundPosition = "center", console.log(`Icon loaded and cached for bonus ${n+1}`)
          })).catch((o => {
            console.error(`Error loading bonus icon [${e}]`, o), imageCache.bonusIcons[e] = { loadError: !0 }, t.style.backgroundImage = "", t.innerHTML = defaultBonusSVG
          })))
        } else t.style.backgroundImage = "", t.innerHTML = defaultBonusSVG;
      else t.style.backgroundImage = "", t.innerHTML = lockedBonusSVG
    };
    i();
    new MutationObserver(i).observe(e, { attributes: !0, attributeFilter: ["class"] })
  }))
}

function cleanupImageCache() {
  Object.values(imageCache.bonusIcons).forEach((o => { o.objectURL && URL.revokeObjectURL(o.objectURL) })), imageCache.bonusIcons = {}
}

function adjustIconsChecker() {
  let o = getVersionFromURL();
  document.getElementById(`icon${o}`).classList.add("open")
}

function fixPictoSizeSprite() {
  const o = app.animearray.length - 1;
  for (let n = 0; n <= o; n++) {
    const e = document.querySelector(`#box-bottom #box-picto .picto#picto${n} .bck`);
    e && (e.style.backgroundPosition = `calc((100% / ${o}) * ${n}) 0`)
  }
}

function modLoaded() {
  initBonusesIcons(), initPictoSize(), initUpdatedPictoIcons(), displayNewMods(), app.recmaxloop = app.maxrecloop, initAmbience(), afterModMods(), initBonusSoundSystem(), fixPictoSizeSprite()
}

function afterModMods() {}

function waitForElement(o, n) {
  const e = setInterval((() => {
...
function changeFavicon(o) {
  ...
  n || (n = document.createElement("link"), n.rel = "icon", document.head.appendChild(n)), n.href = o
}
waitForElement("#home-bt-play", (o => {
  const n = new MutationObserver((e => {
    for (const t of e)
      if ("attributes" === t.type && "class" === t.attributeName && o.classList.contains("animate")) { n.disconnect(), modLoaded() }
  }));
  n.observe(o, { attributes: !0 })
})), OnDelayIn(500, (function() {
  let o = getVersionFromURL();
  changeFavicon(`${app.folder}/icon.png`), o && "Unknown" !== o && app.name !== RegisterMod ? document.title = `${app.name} | ${RegisterMod}` : document.title = `${RegisterMod}`
}));
...
function replaceBackgroundSprite(o, n = 500) {
  const e = document.getElementById("bck-current"),
    t = document.getElementById("bck-next");
  if (null !== currentFadeTimeout && (clearTimeout(currentFadeTimeout), currentFadeTimeout = null), 0 === n) return e.style.transition = "none", t.style.transition = "none", e.style.opacity = 1, t.style.opacity = 0, e.style.backgroundImage = `url(${o})`, void(isTransitioningB = !1);
  transitionPath = o, transitionDuration = n, transitionStartTime = Date.now(), transitionTimeRemaining = n;
  parseFloat(getComputedStyle(e).opacity);
  const i = parseFloat(getComputedStyle(t).opacity);
  isTransitioningB ? (e.style.transition = "none", t.style.transition = "none", e.style.backgroundImage = t.style.backgroundImage, e.style.opacity = i, t.style.opacity = 0) : (t.style.transition = "none", t.style.opacity = 0), t.style.backgroundImage = `url(${o})`, isPaused || (t.offsetWidth, e.style.transition = `opacity ${n}ms ease`, t.style.transition = `opacity ${n}ms ease`, t.style.opacity = 1, e.style.opacity = 0, isTransitioningB = !0, currentFadeTimeout = setTimeout((() => { finishBackgroundTransition() }), n))
}

function finishBackgroundTransition() {
  const o = document.getElementById("bck-current"),
    n = document.getElementById("bck-next");
  o.style.transition = "none", o.style.backgroundImage = n.style.backgroundImage, o.style.opacity = 1, n.style.transition = "none", n.style.opacity = 0, currentFadeTimeout = null, isTransitioningB = !1, transitionPath = null
}

function pauseAllDelays() {
  if (!isPausedD && (isPausedD = !0, pauseStartTime = Date.now(), activeDelays.forEach((o => { o.pause() })), isTransitioningB && null !== currentFadeTimeout)) {
    const o = Date.now() - transitionStartTime;
    transitionTimeRemaining = Math.max(0, transitionDuration - o), clearTimeout(currentFadeTimeout), currentFadeTimeout = null;
    const n = document.getElementById("bck-current"),
      e = document.getElementById("bck-next"),
      t = parseFloat(getComputedStyle(n).opacity),
      i = parseFloat(getComputedStyle(e).opacity);
    n.style.transition = "none", e.style.transition = "none", n.style.opacity = t, e.style.opacity = i
  }
}

function resumeAllDelays() {
  if (isPausedD && (isPausedD = !1, pauseStartTime = null, activeDelays.forEach((o => { o.resume() })), isTransitioningB && null !== transitionPath)) {
    const o = document.getElementById("bck-current"),
      n = document.getElementById("bck-next");
    parseFloat(getComputedStyle(o).opacity), parseFloat(getComputedStyle(n).opacity);
    n.offsetWidth, o.style.transition = `opacity ${transitionTimeRemaining}ms ease`, n.style.transition = `opacity ${transitionTimeRemaining}ms ease`, n.style.opacity = 1, o.style.opacity = 0, transitionStartTime = Date.now(), currentFadeTimeout = setTimeout((() => { finishBackgroundTransition() }), transitionTimeRemaining)
  }
}

function setBackgroundLayer(o) { document.getElementById("box-stage-bck").style.setProperty("--layer-image", `url(${o})`) }

function setPictoLayer(o, n = null) {
  let e = document.querySelectorAll(".picto .bck");
  null === n ? e.forEach((n => { n.style.setProperty("--layer-image", `url(${o})`) })) : n.forEach((n => { n >= 0 && n < e.length && e[n].style.setProperty("--layer-image", `url(${o})`) }))
}

function replacePictoSprite(o) {
  document.querySelectorAll(".picto .bck").forEach((n => { n.style.backgroundImage = `url(${o})` }))
}

function initUpdatedPictoIcons() {
  document.querySelectorAll(".picto").forEach((o => {
    const n = parseInt(o.getAttribute("data-picto-num"), 10),
      e = app.animearray[n];
    if (e && e.icon && "" !== e.icon.trim()) {
      const n = o.querySelector(".bck");
      n && (n.style.backgroundImage = `url('${app.folder}img/${e.icon}')`)
    }
  }))
}

function stageFadeOut(o, n, e = !1, t = null) {
  const i = document.getElementById("box-stage-bck");
  if (!i) return;
  const s = i.querySelector(".stage-fade");
  s && s.remove();
  const l = document.createElement("div");
  l.className = "stage-fade", l.style.backgroundColor = o, l.style.opacity = e ? "0" : "1", l.style.transition = n > 0 ? `opacity ${n}ms ease` : "none", i.appendChild(l), n > 0 ? (l.offsetWidth, l.style.opacity = e ? "1" : "0", e && "function" == typeof t && setTimeout(t, n)) : (l.style.opacity = e ? "1" : "0", e && "function" == typeof t && t())
}
const audioContext = new(window.AudioContext || window.webkitAudioContext);
let isAmbiencePlaying = !1,
  ambiencePath = null,
...
function shouldPlayAmbience() { return !anyPictoIsBusy() && !document.body.classList.contains("darkback") }

async function setAmbience(o) {
  ModSoundsOn && (ambiencePath = `${app.folder}sound/mod/ambience-${o}.ogg`, (isAmbiencePlaying || isTransitioning) && await stopAmbience(), shouldPlayAmbience() && await playAmbience(ambiencePath), checkingInterval || observePictoState())
}

async function playAmbience(o) {
  if (ModSoundsOn && !isTransitioning && shouldPlayAmbience()) {
    isTransitioning = !0;
    try {
      gainNode = audioContext.createGain(), gainNode.gain.setValueAtTime(0, audioContext.currentTime);
      const n = await fetch(o),
        e = await n.arrayBuffer(),
        t = await audioContext.decodeAudioData(e);
      ambienceSource = audioContext.createBufferSource(), ambienceSource.buffer = t, ambienceSource.loop = !0, ambienceSource.connect(gainNode), gainNode.connect(audioContext.destination), ambienceSource.start(), gainNode.gain.linearRampToValueAtTime(1, audioContext.currentTime + FADE_TIME), isAmbiencePlaying = !0
    } catch (o) { console.error("Error in playAmbience:", o) } finally { isTransitioning = !1 }
  }
}

async function stopAmbience(o = !0) {
  if (ambienceSource && gainNode && !isTransitioning) {
    isTransitioning = !0;
    try {
      const o = audioContext.currentTime;
      gainNode.gain.cancelScheduledValues(o), gainNode.gain.setValueAtTime(gainNode.gain.value, o), gainNode.gain.linearRampToValueAtTime(0, o + FADE_TIME), ambienceSource.stop(o + FADE_TIME)
    } catch (o) { console.warn("Error stopping ambience:", o) }
    setTimeout((() => {
      try { ambienceSource.disconnect(), gainNode.disconnect() } catch (o) {}
      ambienceSource = null, gainNode = null, isAmbiencePlaying = !1, isTransitioning = !1, o && checkingInterval && (clearInterval(checkingInterval), checkingInterval = null)
    }), 1e3 * FADE_TIME + 50)
  }
}
...
function anyPictoIsBusy() {
  ...
  return Array.from(o).some((o => o.classList.contains("drag") || o.classList.contains("griser")))
}

function observePictoState() {
  checkingInterval = setInterval((async() => {
    shouldPlayAmbience() ? isAmbiencePlaying || !ambiencePath || isTransitioning || await playAmbience(ambiencePath) : isAmbiencePlaying && await stopAmbience(!1)
  }), 200)
}

function checkAndShowUpdateNotice(o) {
  fetch("https://raw.githubusercontent.com/RemmieUwU/Incredimods/refs/heads/main/buildUpdateData.json").then((o => o.json())).then((n => {
    const e = n.id;
    let t = JSON.parse(localStorage.getItem("shownMessages") || "[]");
    if (t.includes(e)) return;
    const i = n.devVersionOnly,
      s = n.versionForEveryone;
    (i && "Unknow" === o || s) && (boxPopup.open({
      name: "popup-message",
      icntype: "action",
      bodyclose: !0,
      class: "modCreditsBox",
      content: n.content,
      onBoxOpenEnd: function() {
        boxPopup.$popup.find(".icon.bt.bt-round.bt-44").on("click", (() => { boxPopup.close() }))
      },
      onBoxCloseStart: function() {
        boxPopup.$popup.find(".icon.bt.bt-round.bt-44").off()
      }
    }), t.push(e), localStorage.setItem("shownMessages", JSON.stringify(t)))
  })).catch((o => { console.error("Couldn't load update message:", o) }))
}
const forceMod = {
  openApp: function() { forceUserGesture(clickHomeBtPlay) },
  openPlayList: function() { forceUserGesture(clickHomeBtPlaylist) },
  killAllPolos: function() { immediateKillAllPolo(), stopLoop(), clearAllDelays() }
};

function findInScript(o) {
  let n = "";
  for (var e in window) e.toLowerCase().includes(o) && (n += `<p>${e}: ${window[e]}</p>`);
  boxPopup.open({
    name: "popup-message",
    icntype: "action",
    bodyclose: !0,
    class: "modCreditsBox",
    content: `<div class="info-debug">${n}</div>`,
    onBoxOpenEnd: function() {
      boxPopup.$popup.find(".icon.bt.bt-round.bt-44").on("click", (() => { boxPopup.close() }))
    },
    onBoxCloseStart: function() {
...
let shakingIntervals = [],
  wiggleAnimationFrame = null;
const pictoTransforms = new Map;

function updateTransform(o) {
  const n = pictoTransforms.get(o) || { shake: { x: 0, y: 0 }, wiggle: { x: 0, y: 0 } },
    e = n.shake.x + n.wiggle.x,
    t = n.shake.y + n.wiggle.y;
  o.style.transform = `translate(${e}px, ${t}px)`
}

function initShakingPicto(o, n) {
  document.querySelectorAll(".picto").forEach((e => {
    const t = e.querySelector(".bck");
    if (t) {
      const e = setInterval((() => {
        const n = (Math.random() - .5) * o,
          e = (Math.random() - .5) * o,
          i = pictoTransforms.get(t) || { shake: { x: 0, y: 0 }, wiggle: { x: 0, y: 0 } };
        i.shake.x = n, i.shake.y = e, pictoTransforms.set(t, i), updateTransform(t), updateTransform(t)
      }), n);
      shakingIntervals.push({ id: e, bck: t })
    }
  }))
}

function resetShakingPicto(o = .2) {
  shakingIntervals.forEach((n => {
    clearInterval(n.id);
    let e = n.bck,
      t = 4;
    const i = setInterval((() => {
      if (t -= o, t <= 0) {
        clearInterval(i);
        const o = pictoTransforms.get(e) || { x: 0, y: 0 };
        o.x = 0, o.y = 0, pictoTransforms.set(e, o), e.style.transition = "transform 0.3s ease", updateTransform(e), setTimeout((() => { e.style.transition = "" }), 300)
      } else {
        const o = (Math.random() - .5) * t,
          n = (Math.random() - .5) * t,
          i = pictoTransforms.get(e) || { x: 0, y: 0 };
        i.x = o, i.y = n, pictoTransforms.set(e, i), updateTransform(e)
      }
    }), 50)
  })), shakingIntervals = []
}

function wigglePictos(o = 2, n = 10) {
  const e = document.querySelectorAll(".picto");
  let t = null;
  wiggleAnimationFrame = requestAnimationFrame((function i(s) {
    t || (t = s);
    const l = (s - t) / 1e3;
    e.forEach(((e, t) => {
      const i = e.querySelector(".bck");
      if (i) {
        const e = .3 * t,
          s = Math.sin((l - e) * o) * n,
          c = pictoTransforms.get(i) || { shake: { x: 0, y: 0 }, wiggle: { x: 0, y: 0 } };
        c.wiggle.y = s, pictoTransforms.set(i, c), updateTransform(i)
      }
...
function stopWiggle() {
  ...
  document.querySelectorAll(".picto").forEach((o => {
    const n = o.querySelector(".bck");
    if (n) {
      const o = pictoTransforms.get(n) || { x: 0, y: 0 };
      o.y = 0, pictoTransforms.set(n, o), updateTransform(n)
    }
  }))
}
...
function openVotedMods() {
  boxPopup.open({
    name: "popup-message",
    icntype: "action",
    bodyclose: !0,
    class: "modCreditsBox shadow-box",
    content: "Highly rated mods will appear here... Empty for now 🥀",
    onBoxOpenEnd: function() {
      boxPopup.$popup.find(".icon.bt.bt-round.bt-44").on("click", (() => { boxPopup.close() }))
    },
    onBoxCloseStart: function() {
...
OnClick("bt-stop", (function() { clearAllDelays(), QuickMessage("All delays cleaned") }));
let _gamePausedByUser = !1;

function initAmbience() { app.ambience && setAmbience(app.ambience) }

function updateSingleVersionURL() {
  document.getElementById("noYT-prev").style.backgroundImage = `url('./${app.folder}img/home-screen@2x.jpg')`, document.querySelectorAll(".icon-prev").forEach((o => { o.style.backgroundImage = `url('./${app.folder}img/game-picto@2x.png')` }))
}
...
function openAboutPop() {
  boxPopup.open({
    ...
    content: `<div class="modCoverWrapper">\n  <canvas id="modCoverCanvas" width="678" height="282"></canvas>\n</div><div class="mod-info-single">\n    <div class="single-version-icon"></div>\n    <div class="title">${RegisterMod}</div>\n    <div class="txt">${modInfo.description?`${modInfo.description}`:"No info about this mod"}</div>\n    <div class="footer-pop">${modInfo.relatedVideo?`<iframe style="margin-top: 10px;" width="480" height="270" src="${modInfo.relatedVideo.replace("watch?v=","embed/")}"></iframe>`:'<div id="noYT-prev">\n      <div class="icon-prev n2"></div>\n      <div class="icon-prev n1"></div></div>'}</div>\n  </div>`,
    onBoxOpenEnd: function() {
      boxPopup.$popup.find(".icon.bt.bt-round.bt-44").on("click", (function() { boxPopup.close(), document.getElementById("pop-popup").classList.remove("invBor") }));
      const o = document.getElementById("modCoverCanvas"),
        n = o.getContext("2d"),
        e = new Image,
        t = new Image,
        i = new Image;
      const [s, l, c] = (a = 3, [...app.animearray].sort((() => .5 - Math.random())).slice(0, a));
      var a;

      function r(o, n) {
        const e = [`anime/${n}-sprite-hd.png`, `${app.folder}/anime/${n}-sprite-hd.png`, `../anime/${n}-sprite-hd.png`, `../${app.folder}/anime/${n}-sprite-hd.png`, `../../anime/${n}-sprite-hd.png`, `asset/anime/${n}-sprite-hd.png`, `assets/anime/${n}-sprite-hd.png`, `/anime/${n}-sprite-hd.png`, `/${app.folder}/anime/${n}-sprite-hd.png`];
        let t = 0;
        if (window.workingSpritePath) {
          const e = window.workingSpritePath.replace("{name}", n);
          return console.log(`Using known working path for ${n}: ${e}`), void(o.src = e)
        }! function i() {
          if (t >= e.length) return void console.error(`Failed to load sprite: ${n} after trying all paths`);
          const s = e[t];
          console.log(`Trying path for ${n}: ${s}`), o.onload = () => {
            console.log(`SUCCESSFULLY loaded sprite ${n} with path: ${s}`), window.workingSpritePath = s.replace(n, "{name}"), console.log(`Found working path pattern: ${window.workingSpritePath}`)
          }, o.onerror = () => { console.log(`Failed with path: ${s}`), t++, i() }, o.src = s
        }()
      }
      console.log("Loading sprites for:", s.name, l.name, c.name), r(e, s.name), r(t, l.name), r(i, c.name), setTimeout((() => {
        try {
          const s = 328,
            l = 760;
          n.clearRect(0, 0, o.width, o.height);
          const c = n.createLinearGradient(44, 0, 146, o.height);
          c.addColorStop(0, app.col1), c.addColorStop(1, app.col3), n.fillStyle = c, n.fillRect(0, 0, o.width, o.height);
          let a = 0;
          if (e.complete && 0 !== e.naturalWidth && a++, t.complete && 0 !== t.naturalWidth && a++, i.complete && 0 !== i.naturalWidth && a++, a > 0) {
            if (t.complete && 0 !== t.naturalWidth && i.complete && 0 !== i.naturalWidth) {
              const e = .8,
                c = s * e,
                a = l * e,
                r = (o.height - a) / 2 + 127,
                ...
                d = (o.width - c) / 2 - u,
                f = (o.width - c) / 2 + u;
              n.save(), n.globalAlpha = .4, n.filter = "brightness(100%)", n.shadowColor = "rgba(0, 0, 0, 0.5)", n.shadowBlur = 60, n.shadowOffsetX = 0, n.shadowOffsetY = 8;
              try { n.drawImage(t, 0, 0, s, l, d, r, c, a), n.drawImage(i, 0, 0, s, l, f, r, c, a) } catch (o) { console.error("Error drawing side sprites:", o) }
              n.restore()
            }
            const c = n.createLinearGradient(0, 0, 0, o.height);
            if (c.addColorStop(0, "rgba(0, 0, 0, 0)"), c.addColorStop(.5, "rgba(0,0,0,0.3)"), c.addColorStop(1, "rgba(0,0,0,0.6)"), n.fillStyle = c, n.fillRect(0, 0, o.width, o.height), n.fillStyle = "rgba(0, 0, 0, 0.15)", n.fillRect(0, 0, o.width, o.height), e.complete && 0 !== e.naturalWidth) {
              const t = s,
                i = l,
                c = (o.width - t) / 2,
                a = (o.height - i) / 2 + 137;
              n.save(), n.beginPath(), n.rect(c, a, t, i), n.clip();
              try { n.drawImage(e, 0, 0, s, l, c, a, t, i) } catch (o) { console.error("Error drawing front sprite:", o) }
              n.restore()
            }
          }
        } catch (o) { console.error("Error in canvas rendering:", o) }
      }), 1e3)
    },
    onBoxCloseStart: function() {
      boxPopup.$popup.find(".icon.bt.bt-round.bt-44").off()
    },
    onCloseComplete: function() { console.log("Popup closed"), document.getElementById("pop-popup").classList.remove("invBor") }
  }), updateSingleVersionURL()
}
Object.defineProperty(window, "gamePausedByUser", {
  get: () => _gamePausedByUser,
  set(o) {
    o !== _gamePausedByUser && (_gamePausedByUser = o, o ? pauseAllDelays() : resumeAllDelays())
  }
}), document.addEventListener("DOMContentLoaded", (function() {
  window.app = window.app || {}, window.electron ? (app.isElectron = !0, app.electronAppPath = window.electron.paths.appPath, app.folder = "") : (app.isElectron = !1, app.folder = "asset-v1/")
})), onKeyHold("q", "r", (() => { clickBtRecord() })), onKeyHold("w", "r", (() => { clickBtRandom() })), onKeyHold("a", "r", (() => { stopAllStage(), clearAllDelays() }));
let singleVersionMod = !1;

function logDebug(o) { console.log(`[VersionManager] ${o}`) }

function unHide(o) {
  const n = document.getElementById(`icon${o}`);
  if (!n) return !1;
  try {
    return n.style.removeProperty("display"), n.style.display = "block", n.dataset.hidden = "false", logDebug(`unhid icon${o}`), updateSwitchButtonState(), !0
  } catch (n) { return console.error(`Error unhiding icon${o}:`, n), !1 }
}
...
function Hide(o) {
  ...
  if (!n) return !1;
  try {
    return n.style.display = "none", n.dataset.hidden = "true", updateSwitchButtonState(), !0
  } catch (n) { return console.error(`Error hiding icon${o}:`, n), !1 }
}

function updateSwitchButtonState() {
  const o = document.querySelectorAll('[id^="icon"]'),
    n = Array.from(o).filter((o => {
      const n = "none" !== window.getComputedStyle(o).display,
        e = o.classList.contains("vicon");
      return n && e
    }));
  singleVersionMod = 1 === n.length;
  const e = document.querySelector("#home-bt-switch use");
  if (e) {
    const o = singleVersionMod ? "#ic-rate" : "#ic-switch";
    e.setAttribute("xlink:href", o)
  }
}

function setupSwitchButton() {
  logDebug("Setting up switch button"), "function" == typeof OnClick ? OnClick("home-bt-switch", (() => { singleVersionMod ? openAboutPop() : popupSwitch() })) : console.error("OnClick function not available yet")
}

function checkIfModRegistered() { modRegistered = "Unknown" !== RegisterMod }
window.checkAllIcons = function() {
  const o = document.querySelectorAll('[id^="icon"]');
  return console.log(`Found ${o.length} icon elements`), o.forEach((o => {
    const n = o.id,
      e = window.getComputedStyle(o).display;
    console.log(`${n}: display = ${e}`)
  })), "Icon check complete"
}, setTimeout((() => {
  updateSwitchButtonState(), setupSwitchButton(), adjustIconsChecker()
}), 600), OnDelayIn(400, (() => { checkIfModRegistered() }));
...
var RegisterMod = "Alone";
var RegisterModVersion = "1.4.7";
var ModDeveloper = "MrF";

modInfo = {
  description: "This mod for fun",
  relatedVideo: "",
};
